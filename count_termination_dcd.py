		...
		if slicing[1][p]!=slicing[1][p-1]:	
			...
			for q in range(0,np.size(temp,0)):
				...
				if direct_r > direct_cut_off:
					...
					break
	print ("     ")						
	print ("indirect bindings ---> "+ str(b))
	print ("direct bindings   ---> "+ str(total_binds-b))			
	...
	####dwelling time
        ...
        for t in range(0,np.size(slicing,1)):
                ...
                if slicing[1][p]!=slicing[1][p-1]:
                        ...
                        for q in range(-1*int(np.size(temp,0)),0):
                            q=-1*q
                            x=int(temp[q][0])
                            y=int(temp[q][1])
                            z=int(temp[q][2])
                            ...
                            if dwell_r > dwell_cut_off:
                                ...
                                break
	print ("average dwell time steps ---> "+str(s/total_binds))
